...
if __name__ == '__main__':
    ...
    parser.add_argument('mode',
                        choices=['dummy', 'image', 'videos'],
                        default='dummy',
                        help='Run mode. \'dummy\' will send an emtpy buffer to the server to test if inference works. \'image\' will process an image. \'videos\' will process a videos.')
    parser.add_argument('input',
                        type=str,
                        nargs='?',
                        help='Input file to load from in image or videos mode')
    ...
    # VIDEO MODE
    if FLAGS.mode == 'videos':
        print("Running in 'videos' mode")
        if not FLAGS.input:
            print("FAILED: no input videos")
            sys.exit(1)
        ...
        print("Opening input videos stream...")
        cap = cv2.VideoCapture(FLAGS.input)
        if not cap.isOpened():
            print(f"FAILED: cannot open videos {FLAGS.input}")
            sys.exit(1)
        ...
        while True:
            ...
            if counter == 0 and FLAGS.out:
                print("Opening output videos stream...")
